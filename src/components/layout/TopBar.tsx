...
export default function TopBar({ pageTitle, onMenuToggle }: TopBarProps) {
  ...
  return (
    <header
      className="sticky top-0 z-35 flex items-center justify-between h-16 px-6 text-white overflow-hidden"
      style={{
        background: '#002147', // Navy Blue — matching sidebar
        borderBottom: '3px solid #D4AF37',
        boxShadow: '0 8px 40px rgba(0, 0, 0, 0.4)',
      }}
    >
      {/* Bold gold ambient glow */}
      <div className="absolute inset-x-0 top-0 h-full bg-gradient-to-b from-[#D4AF37]/8 to-transparent pointer-events-none z-0" />
      ...
      <div className="flex items-center gap-4 min-w-0 relative z-10">
        ...
        <button
          ...
          className="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center text-slate-500 transition-all duration-200 hover:text-slate-900 hover:bg-slate-100"
              ...
              <span key={crumb.path} className="flex items-center gap-1.5">
                {index === 0
                  ? (
                    <button
                      onClick={() => navigate('/')}
                      className="text-slate-500 hover:text-[#D4AF37] transition-colors duration-150"
                    >
                      <Home size={13} />
                    </button>
                  )
                  ...
                  : (
                    <button
                      ...
                      className="text-slate-500 hover:text-[#D4AF37] transition-colors duration-150 truncate max-w-[120px]"
                    >
                      {crumb.label}
                    </button>
                  )
      ...
      <div className="flex items-center gap-2">
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="relative w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 transition-all duration-200 hover:text-[#D4AF37] hover:bg-white/10"
